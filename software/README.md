...
密钥字段（`api_key` / `device_secret`）落盘为 `enc1:` 密文，由本机 CPU 序列号派生，换板不可解。设置页「API 密钥 / API 网关口令」保存后为空，眼睛图标可在填写时核对明文。「API 网关地址」明文显示，不加密。空密钥表示保持原密文；网关地址空栏保持原主机列表。

语音切场口令整句等于「新对话 / 新会话 / 新建对话 / 新建会话」，以及 ASR 常把「会话」听成的「新绘画 / 新建绘画」。
